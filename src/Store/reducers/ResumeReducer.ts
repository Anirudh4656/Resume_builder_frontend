...
export type ResumeSection = Exclude<
  keyof ResumeState,
  "title" | "template" | "image" | "personal">;
...
const resumeSlice = createSlice({
  ...
  reducers: {
    ...
    addSectionItem: <T extends ResumeSection>(
      state: ResumeState,
      action: PayloadAction<AddSectionItemPayload<T>>
    ) => {
      const { section, item } = action.payload;
      (state[section] as any[]).push(item);
    },
    updateSectionItem: <T extends ResumeSection>(
      state: ResumeState,
      action: PayloadAction<UpdateSectionItemPayload<T>>
    ) => {
